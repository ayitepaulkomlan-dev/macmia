...
/** Champs qui comptent dans la complétude, avec leur libellé pour le détail. */
...
/** Complétude réelle, calculée sur ce qui a été extrait — jamais un chiffre décoratif. */
    ...
    <div className="rise" style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div className="card card-xl">
        <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{ font: "800 20px/1.1 var(--font)", color: 'var(--ink)' }}>
              {profil.nom || 'Nom non détecté'}
            </div>
            <div style={{ fontSize: 13.5, color: 'var(--muted)', marginTop: 4 }}>
              {[profil.poste_actuel, profil.annees_experience ? `${profil.annees_experience} ans d’expérience` : '']
                .filter(Boolean)
                .join(' · ') || 'Poste non détecté'}
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '22px 0 8px' }}>
          ...
          <span style={{ fontSize: 12.5, fontWeight: 800, color: pct >= 70 ? 'var(--green)' : 'var(--amber)' }}>
            {pct}%
          </span>
        </div>
        ...
        {missing.length > 0 && (
          <div style={{ fontSize: 11.5, color: 'var(--muted-2)', marginTop: 9 }}>
            Non détecté dans le document : {missing.join(', ')}.
          </div>
        )}
      </div>
      ...
      {degraded && (
        ...
        >
          ...
          <div style={{ fontSize: 12.5, lineHeight: 1.5 }}>
            <strong>Lecture sans modèle de langue.</strong> Le service Ollama n’a pas répondu : les
            compétences ont été relevées par règles, ce qui en laisse passer certaines. Démarrez
            Ollama puis relancez l’analyse pour un relevé complet.
          </div>
        </div>
      )}
        ...
        <div className="card">
          ...
          <div className="meta-row">
            ...
            <span className="meta-val">
              {profil.annees_experience ? `${profil.annees_experience} ans` : '—'}
            </span>
          </div>
          <div className="meta-row">
          ...
          {(profil.diplomes || []).map((d, i) => (
            <div key={i} style={{ padding: '12px 0', borderBottom: '1px solid var(--line-3)' }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink)' }}>{d.titre}</div>
              {(d.etablissement || d.annee) && (
                <div style={{ fontSize: 11.5, color: 'var(--muted-2)', marginTop: 3 }}>
                  {[d.etablissement, d.annee || null].filter(Boolean).join(' · ')}
                </div>
              )}
            </div>
          ))}
        ...
        <div className="card">
          ...
          {(profil.competences_brutes || []).length === 0 ? (
            <div style={{ fontSize: 12.5, color: 'var(--muted-2)', lineHeight: 1.5 }}>
              Aucune compétence relevée. Vérifiez que votre CV comporte une section
              « Compétences » clairement titrée.
            </div>
          ) : (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {profil.competences_brutes.map((c, i) => (
        ...
        <div className="card">
          <div style={{ marginBottom: 4 }}>
            ...
            <p style={{ fontSize: 12.5, color: 'var(--muted)', marginTop: 6, lineHeight: 1.5 }}>
              Vos compétences rapprochées du référentiel France Compétences. Les libellés sont
              ceux de votre CV, jamais reformulés.
            </p>
          </div>
      ...
      {profil.meta && (
        <details className="card" style={{ padding: '16px 22px' }}>
          <summary style={{ cursor: 'pointer', font: "700 12.5px/1 var(--font)", color: 'var(--ink-3)' }}>
            Ce que le lecteur a vu dans le document
          </summary>
          <div style={{ marginTop: 14, fontSize: 12, color: 'var(--muted)', lineHeight: 1.7 }}>
            {profil.meta.pages} page(s) · {profil.meta.blocs_ocr} blocs de texte ·{' '}
            {profil.meta.caracteres} caractères · relevé par{' '}
            {profil.source === 'llm' ? 'modèle de langue' : 'règles'}
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 12 }}>
              {(profil.meta.sections || []).map((s, i) => (
                <span
                  key={i}
                  className={`chip ${s.confiance === 'semantic' ? 'chip-green' : 'chip-gray'}`}
                  title={s.confiance === 'semantic' ? 'Titre de section reconnu' : 'Détecté à la mise en forme'}
                >
                  {s.titre || s.categorie}
                </span>
              ))}
